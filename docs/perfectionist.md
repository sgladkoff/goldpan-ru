...
Эта кнопка вызывает простое диалоговое окно, в котором предлагается казать имя создаваемого проекта.

![perf1](perfN1.png)

Созданный проект появляется в общей таблице проектов на странице **Requestor Room**. Каждая строка таблицы описывает отдельный проект с указанием его вида и количества вложенных заданий (файлов), а также позволяет **Заказчику** загрузить файл с результатами проекта или удалить проект.

Нажатие на название проекта, в который еще не были добавлены задачи, открывает окно **New Job** (Новые задачи). Кнопка **Create** (Создать) вызывает дополнительное окно, в которое можно перетащить билингвальные файлы подходящего формата; нажатие на пространство этого окна также открывает стандартный диалог поиска файлов Explorer.

В окне **New Job** также содержится ссылка для загрузки файла sample.xlsx.

![perf2](perfN2.png)

При загрузке файла в сервис из него исключаются повторяющиеся сегменты переводов.

Если проект уже содержит задачи, нажатие на его название переносит **Заказчика** на собственную страницу проекта. На ней все файлы, добавленные в проект, представлены в таблице заданий, а также присутствует возможность добавлений новых заданий с помощью кнопки **Create Task** (Создать задание).
...
Если задача выполнена полностью, **Заказчик** завершает ее нажатием на кнопку **Finish Review** (Завершить оценку) справа снизу.
...
Если выбран вид задания **Translation** (Перевод), то поле **Your Input** (Ваш вариант) в сегментах тестового файлеа будет пустым, и испытуемый должен будет заполнять его. Если выбран вид задания **Editing** (Редактура), поле будет содержать текст для редактуры.


![perf13](perfN13.png)

После того, как испытуемый нажмет на кнопку **Finish Task** (Завершить задание), заверщенное тестовое задание можно будет назначить **Рецензенту** для проверки, как обычный LQA-проект. 
...
**Рецензент** оперирует страницей **Reviewer Room** (https://lqa.amberites.ru/reviewer-room). На ней представлена таблица, содержащая все задачи, назначенные в адрес конкретного **Рецензента** **Заказчиками**.

![perf18](perfN18.png)

**Рабочая область** задачи открывается нажатием на ее название в таблице задач. Она содержит таблицу для рецензирования со столбцами **Source** (Исходный текст), **Target** (Перевод), **Errors** (Ошибки), **Fixed Target** (Исправленный перевод) и флажком **Reviewed** (Оценено), который выставляется при вносе ошибки либо вручную.
...
Переход на страницу проекта осущетсвляется нажатием на его название в списке. В проекте **Наблюдателям** видны только те задачи, которые уже были назначены **Рецензентам**.
...
Как только **Рецензент** отметит задание как выполненное, **Заказчик** (как и приглашенные **Наблюдатели**) увидит на странице задания результаты экспертизы и сможет скачать таблицу формата **XLSX** с результатами, представленными согласно выбранной метрике. Загрузка доступна по нажатию на символы "скачать" в таблице задач проекта.